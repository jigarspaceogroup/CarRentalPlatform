...
function mapFilterToStatus(filter: FilterTab): BookingStatus | 'ALL' {
...
export default function BookingsScreen() {
  ...
  const { bookings, isLoading, error, refetch, filterByStatus, activeFilter } =
    useBookings();
  ...
  const handleBookingPress = (booking: Booking) => {
    router.push(`/(tabs)/booking-detail/${booking.id}`);
  };

  const handleFilterChange = (tab: FilterTab) => {
    filterByStatus(mapFilterToStatus(tab));
  };

  const getActiveFilterTab = (): FilterTab => {
    if (activeFilter === 'ALL') return 'ALL';
    return activeFilter as FilterTab;
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + spacing.lg }]}>
      ...
      <View style={styles.filterContainer}>
        {FILTER_TABS.map((tab) => {
          const isActive = getActiveFilterTab() === tab;
          return (
            <TouchableOpacity
              key={tab}
            ...
            >
              ...
              >
                {t(`bookings.filter.${tab.toLowerCase()}`)}
              </Text>
            </TouchableOpacity>
          );
        })}
